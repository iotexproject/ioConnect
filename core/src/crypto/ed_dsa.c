#include "include/backends/tinycryt/constants.h"
#include "include/backends/tinycryt/sha512.h"
#include "include/backends/tinycryt/ge.h"
#include "include/backends/tinycryt/sc.h"

static int consttime_equal(const unsigned char *x, const unsigned char *y) 
{
    unsigned char r = 0;

    r = x[0] ^ y[0];
    #define F(i) r |= x[i] ^ y[i]
    F(1);
    F(2);
    F(3);
    F(4);
    F(5);
    F(6);
    F(7);
    F(8);
    F(9);
    F(10);
    F(11);
    F(12);
    F(13);
    F(14);
    F(15);
    F(16);
    F(17);
    F(18);
    F(19);
    F(20);
    F(21);
    F(22);
    F(23);
    F(24);
    F(25);
    F(26);
    F(27);
    F(28);
    F(29);
    F(30);
    F(31);
    #undef F

    return !r;
}

int uECC_ed25519_sign(const uint8_t *private_key, const uint8_t *public_key, const uint8_t *message, unsigned message_len, uint8_t *signature)
{
	struct tc_sha512_state_struct s;

	unsigned char hram[64];
    unsigned char r[64];
    ge_p3 R;	

    tc_sha512_init(&s);
    tc_sha512_update(&s, private_key + 32, 32);
    tc_sha512_update(&s, message, message_len);
    tc_sha512_final(&s, r);	

    sc_reduce(r);
    ge_scalarmult_base(&R, r);
    ge_p3_tobytes(signature, &R);

    tc_sha512_init(&s);
    tc_sha512_update(&s, signature, 32);
    tc_sha512_update(&s, public_key, 32);
    tc_sha512_update(&s, message, message_len);
    tc_sha512_final(&s, hram);		

    sc_reduce(hram);
    sc_muladd(signature + 32, hram, private_key, r);

	return 1;	
}

int uECC_ed25519_verify(const uint8_t *public_key, const uint8_t *message, unsigned message_len, const uint8_t *signature) 
{
	struct tc_sha512_state_struct s;

    unsigned char h[64];
    unsigned char checker[32];
    ge_p3 A;
    ge_p2 R;

    if (signature[63] & 224) {
        return 0;
    }

    if (ge_frombytes_negate_vartime(&A, public_key) != 0) {
        return 0;
    }

    tc_sha512_init(&s);
    tc_sha512_update(&s, signature, 32);
    tc_sha512_update(&s, public_key, 32);
    tc_sha512_update(&s, message, message_len);
    tc_sha512_final(&s, h);
    
    sc_reduce(h);
    ge_double_scalarmult_vartime(&R, h, &A, signature + 32);
    ge_tobytes(checker, &R);

    if (!consttime_equal(checker, signature)) {
        return 0;
    }

    return 1;
}

